                ...
                <table class="table table-bordered table-hover display">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <th colspan="2">
                                <input type="number" class="form-control" v-model="discount_2" min='0' max='100' oninput="if(this.value.length == 0) this.value=0">
                            </th>
                        ...
                        <tr>
                            ...
                            <th colspan="2">
                                {{ return_money > 0 ? parseFloat(return_money).toFixed(2) : 0 }}
                            </th>
                        </tr>
                    </tbody>
                </table>
...
export default {
    ...
    'paid_field', 'customer_mobile_field','table_no_old','no_data', 'after_discount', 'after_vat', 'return_field', 'payable_field'
    ],
    data() {
        return {
            active: false,
    ...
    methods: {
        ...
        addToSaleProduct: function () {
            ...
            if (this.table_no != 0) {
                axios.post(this.url + '/admin/sale/update-sale-product/', {
                ...
                .then((response) => {
                    window.location.href = this.url + "/admin/sale/view/" + response.data;
                })
                .catch((exception) => {
                    console.log(exception);
                });
        ...
        allCartData: function(){
            axios.get(this.url + '/admin/sale/all-cart-item/'+this.table_no_old)
            .then((response) => {
                if (response.data.count > 0) {
                    //get cart options
                    let cartOptions = Object.values(response.data.cart)[0].options;
                    this.customer_name = cartOptions.customer_name;
                    this.table_no = cartOptions.table_no;
                    this.mobile = cartOptions.customer_mobile;
                    this.given_money = cartOptions.given_money;
                    this.discount = cartOptions.discount; 
                    this.discount_2 = cartOptions.discount_2; 
                    this.vat = cartOptions.vat;
                    this.sale_id = cartOptions.sale_id;

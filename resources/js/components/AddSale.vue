...
export default {
    ...
    'action', 'url', 'customer_name_field', 'paid_field', 'customer_mobile_field','no_data', 'after_discount', 'after_vat', 'paid_field', 'return_field',
    'payable_field'
    ],
    data(){
        return {
        ...
        addToSale: function(product){
            if(this.$store.state.table_no > 0){
                axios.post(this.url+'/admin/sale/add-to-sale', {
                    product: product,
                    table_no: this.$store.state.table_no,
                    customer_mobile: this.$store.state.mobile,
                    customer_name: this.$store.state.customer_name,
                })
                .then((response) => {

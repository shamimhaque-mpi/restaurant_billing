  ...
  <div class="col-md-12">
    <div class="card">
      <div class="card-header">
        <div class="row">
          <div class="col-md-6"><h2><i class="fa fa-pencil-square"></i> {{ __('backend/brand.edit_brand') }}</h2></div>
      ...
      <div class="card-body">
        ...
        <form class="form-horizontal" id="myform" action="{{ route('admin.brand.update', $brand->slug) }}" method="post" enctype="multipart/form-data">
          @csrf
          ...
          <div class="form-group row">
            ...
            <div class="col-md-5">
              <input id="title" class="form-control" type="text" value="{{  $brand->title }}" name="title"  >
            </div>
          </div>
          ...
          <div class="form-group row">
            ...
            <div class="col-md-5">
              <select id="category_id" class="form-control" type="text" name="category_id"  >
                <option value="" disabled>{{ __('backend/form_field.select_category') }}</option>
                @foreach($category as $key => $row)
                <option {{ $brand->category_id == $row->id ? 'selected' : '' }} value="{{ $row->id }}">{{ $row->title }}</option>
                @endforeach
              </select>
            </div>
          </div>
          ...
          <div class="col-md-5">
            <select id="sub_category_id" class="form-control" type="text" name="sub_category_id"  >
              <option value="" disabled>{{ __('backend/form_field.select_subcategory') }}</option>
              @foreach($subCategory as $key => $row)
              <option {{ $brand->category_id == $row->sub_category_id ? 'selected' : '' }} value="{{ $row->id }}">{{ $row->title }}</option>
              @endforeach
            </select>
          </div>
        </div>
